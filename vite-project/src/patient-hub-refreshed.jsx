...
function normalizePatient(value) {
  ...
  return {
    ...
    condition: typeof raw.condition === "string" ? raw.condition : "General",
    doctor: typeof raw.doctor === "string" ? raw.doctor : "",
    ...
    medications: Array.isArray(raw.medications)
      ? raw.medications.map((m, i) => ({
          ...
          refill: m?.refill || "Review with doctor",
    ...
    reports: Array.isArray(raw.reports)
      ? raw.reports.map((r, i) => ({
          id: r?._id || r?.id || i + 1,
          name: r?.name || "Medical report",
          date: r?.date || "Not specified",
          status: r?.status || "Pending review",
          value: r?.value || "See report",
        }))
      : [],
  };
}
...
function usePatient() {
  ...
  if (!patient) throw new Error("Patient data is not available.");
  return patient;
}
    ...
    <div>
      <Card className="p-8 md:p-10 mb-8 overflow-hidden relative" style={{ background: "linear-gradient(135deg, var(--mint-soft), var(--sky-soft))" }}>
        ...
        <div className="relative">
          <Pill_ tone="mint">{usePatient().condition}</Pill_>
          <h1 className="ph-head text-3xl md:text-4xl font-semibold mt-4 mb-2 text-[var(--ink)]">
            Good afternoon, {usePatient().name.split(" ")[0]}
          </h1>
      ...
      <div className="grid md:grid-cols-5 gap-6">
        ...
        <Card className="p-6 md:col-span-2">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <p className="text-sm font-semibold">{usePatient().doctor}</p>
...
function AppointmentSection() {
  ...
  const bookAppointment = async () => {
    if (!slot || !patient._id) return;
    setBusy(true);
    setError("");
    try {
      await apiRequest("/appointments", {
        method: "POST",
        body: JSON.stringify({ patientId: patient._id, slot, status: "Booked" }),
      });
      setConfirmed(true);
    } catch (err) {
      ...
      {confirmed ? (
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <Card className="p-5 h-fit">
            ...
            {error && <p className="text-xs text-[var(--red)] bg-[var(--peach-soft)] rounded-xl px-3 py-2 mb-3">{error}</p>}
            <button
              disabled={!slot || busy}
...
function ConsultationSection() {
  ...
  useEffect(() => {
    ...
    apiRequest(`/messages/${patient._id}`).then(setMessages).catch(() => {});
  }, [patient._id]);

  const sendMessage = async () => {
    if (!message.trim() || !patient._id) return;
    setSending(true);
    try {
      const created = await apiRequest("/messages", { method: "POST", body: JSON.stringify({ patientId: patient._id, content: message.trim() }) });
    ...
    <div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2 p-6 md:p-8 text-center" style={{ background: "linear-gradient(160deg, var(--lavender-soft), var(--sky-soft))" }}>
          ...
          <p className="ph-head text-xl font-semibold mb-1">{usePatient().doctor}</p>
          ...
          {active && (
            <div className="mt-6 rounded-2xl bg-white p-4 text-left flex items-center gap-3">
              ...
              <p className="text-sm font-medium">Connecting you to {usePatient().doctor.split(" ").slice(-1)[0]}…</p>
        ...
        <Card className="p-6">
          ...
          <div className="space-y-3 mb-4 text-sm max-h-48 overflow-y-auto">
            {messages.length === 0 && <p className="text-xs text-[var(--ink-soft)]">No messages yet. Start a conversation with your care team.</p>}
            {messages.map((m) => (
              <div key={m._id || `${m.timestamp}-${m.content}`} className="rounded-2xl px-3 py-2 bg-gray-100 max-w-[85%] ml-auto text-right">{m.content}</div>
            ))}
          </div>
          <div className="flex gap-2">
            <input value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendMessage()} placeholder="Type a message…" className="flex-1 rounded-full px-4 py-2 text-sm bg-gray-100 outline-none" />
            <button disabled={sending} onClick={sendMessage} className="ph-button w-9 h-9 rounded-full bg-[var(--burgundy)] flex items-center justify-center text-white shadow-md hover:brightness-110 transition disabled:opacity-50"><ChevronRight size={16} /></button>
          </div>
        </Card>
      </div>
    </div>
...
function PrescriptionsSection() {
  ...
  return (
    <div>
      <SectionTitle eyebrow="Current medication" title="Prescriptions" sub={`Prescribed by ${patient.doctor || "your care team"}.`} />
...
function ReportsSection() {
  const reports = usePatient().reports || [];
  return (
    <div>
...
function DietSection() {
  const conditionKey =
    usePatient().condition?.toLowerCase().includes("diabet") ? "Diabetes" :
    usePatient().condition?.toLowerCase().includes("hypertension") ? "Hypertension" :
    "General";

  const plan = DIET_PLANS[conditionKey] || DIET_PLANS.General;
  const customDiet = (usePatient().diet || "").trim();
  ...
  return (
    <div>
      <SectionTitle
        ...
        sub={`A care plan based on the patient's condition: ${usePatient().condition}.`}
      />
      ...
      <Card className="p-5 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-lg font-semibold">{usePatient().condition}</p>
          </div>
        </div>
      </Card>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <Card className="p-6">
          ...
          <p className="text-xs text-[var(--ink-soft)] mt-5">
            Plan prepared for {usePatient().condition} · reviewed by {DOCTORS[3].name}
          </p>
        </Card>
      </div>
    </div>
...
function AdminSection({ onLogout }) {
  const emptyPatient = { _id: "", name: "", email: "", password: "", condition: "General", doctor: "", diet: "", medications: [], reports: [] };
  const [current, setCurrent] = useState(emptyPatient);
  ...
  const savePatient = async (e) => {
    ...
    try {
      const savedPatient = current._id
        ? await apiRequest(`/patients/${current._id}`, { method: "PUT", body: JSON.stringify(patient) })
        : await apiRequest("/auth/register", { method: "POST", body: JSON.stringify(patient) });

      setCurrent(normalizePatient(savedPatient));
      setForm((prev) => ({ ...prev, ...patient }));
      setSaved(true);
      setTimeout(() => setSaved(false), 2600);
      setActiveView("patients");
    } catch (err) {
      setError(err.message || "The patient record could not be saved.");
    } finally {
      setBusy(false);
    }
  };
    ...
    <form onSubmit={savePatient} className="space-y-6">
      ...
      <div className="sticky bottom-4 z-10 rounded-2xl bg-white/90 backdrop-blur border border-black/5 shadow-lg p-3 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button disabled={busy} type="submit" className="ph-button px-5 py-2.5 rounded-xl bg-[var(--burgundy)] text-white font-semibold flex items-center gap-2 disabled:opacity-50">
            <Save size={17} /> {busy ? "Saving…" : "Save patient"}
          </button>
        </div>
      </div>
    </form>
...
/* ---------------------------------------------------------
   Login
--------------------------------------------------------- */
function LoginPage({ onPatientLogin, onAdminLogin }) {
  ...
  const submit = async (e) => {
    ...
    try {
      if (role === "admin") {
        // The supplied backend has no admin model/route, so this remains the UI admin credential.
        if (email.trim().toLowerCase() !== "admin@carehub.demo" || password !== "admin123") {
          throw new Error("Incorrect admin email or password.");
        }
        onAdminLogin();
      } else {
        const patient = await apiRequest("/auth/login", {
          method: "POST",
          body: JSON.stringify({ email: email.trim().toLowerCase(), password }),
        });
        onPatientLogin(normalizePatient(patient));
      }
    } catch (err) {
      ...
      <div className="w-full max-w-md ph-login-card">
        ...
        <Card className="p-6 md:p-7">
          <div className="flex p-1 bg-[var(--sky-soft)] rounded-2xl mb-6">
            <button type="button" onClick={() => { setRole("patient"); setError(""); }} className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition ${role === "patient" ? "bg-white shadow-sm text-[var(--ink)]" : "text-[var(--ink-soft)]"}`}>
              <UserRound size={16} className="inline mr-2" /> Patient
            </button>
            <button type="button" onClick={() => { setRole("admin"); setError(""); }} className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition ${role === "admin" ? "bg-white shadow-sm text-[var(--ink)]" : "text-[var(--ink-soft)]"}`}>
              <ShieldCheck size={16} className="inline mr-2" /> Admin
            </button>
          </div>
          <form onSubmit={submit} className="space-y-4">
            <div className="ph-login-field">
              <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" required placeholder={role === "admin" ? "admin@carehub.demo" : "patient@example.com"} className="w-full rounded-xl px-4 py-3 text-sm bg-[var(--sky-soft)] outline-none border border-transparent focus:border-[var(--burgundy)] transition" />
            </div>
            <div className="ph-login-field">
            ...
            {error && <p className="text-sm text-[var(--red)] bg-[var(--peach-soft)] rounded-xl px-3 py-2">{error}</p>}
            <button disabled={busy} type="submit" className="ph-button w-full py-3 rounded-xl bg-[var(--burgundy)] text-white font-semibold disabled:opacity-50">
              {busy ? "Connecting…" : `Log in as ${role === "admin" ? "Admin" : "Patient"}`}
            </button>
          </form>
          <div className="mt-5 rounded-2xl bg-[var(--mint-soft)] p-4 text-xs text-[var(--ink-soft)]">
            <p className="font-semibold text-[var(--ink)] mb-1">Access</p>
            {role === "admin" ? <p>Admin: <span className="font-semibold">admin@carehub.demo</span> · <span className="font-semibold">admin123</span></p> : <p>Use the patient email and password registered in MongoDB.</p>}
          </div>
        </Card>
      </div>
    </div>
      ...
      return (
        <div className="min-h-screen flex items-center justify-center p-6 bg-[#f8f7f7]">
          <div className="max-w-md w-full bg-white rounded-3xl p-7 shadow-xl border border-black/5 text-center">
            ...
            <p className="text-sm text-gray-500 mt-2">The page hit an unexpected UI error. Your patient record is stored in the CareHub backend.</p>
...
function App() {
  ...
  if (!role) {
    return (
      <LoginPage
        onPatientLogin={(loggedInPatient) => { setPatient(loggedInPatient); setRole("patient"); }}
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-5 md:py-7">
          <header className="flex items-center justify-between mb-6 md:mb-8 bg-white/75 backdrop-blur rounded-3xl border border-black/5 px-4 md:px-5 py-3 shadow-[0_10px_30px_rgba(7,18,47,.05)]">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => { setRole(null); setPatient(null); }} className="px-3 py-2 rounded-xl text-sm font-semibold text-[var(--ink-soft)] hover:bg-[var(--sky-soft)] transition flex items-center gap-2">
  ...
  return (
    <PatientContext.Provider value={patient}>
      <div className="ph-root min-h-screen" data-patient-email={patient?.email || ""}>
        <Fonts />
      ...
      <div className="flex">
        <aside className="hidden md:flex flex-col w-64 shrink-0 h-screen sticky top-0 border-r border-black/5 bg-white/60 backdrop-blur px-5 py-6">
          ...
          <div className="rounded-2xl p-4 mt-4" style={{ background: "var(--peach-soft)" }}>
            ...
            <p className="text-sm font-semibold">{usePatient().name}</p>
            <p className="text-xs text-[var(--ink-soft)] mt-1">{usePatient().condition}</p>
          </div>
          <button onClick={() => { setRole(null); setPatient(null); }} className="mt-3 px-3 py-2 rounded-xl text-xs font-semibold text-[var(--ink-soft)] hover:bg-white transition flex items-center gap-2">
        ...
        {mobileOpen && (
          <div className="ph-overlay-enter fixed inset-0 z-30 bg-black/30 md:hidden" onClick={() => setMobileOpen(false)}>
            <div className="ph-menu-enter absolute right-0 top-0 bottom-0 w-64 bg-white p-5" onClick={(e) => e.stopPropagation()}>
              ...
              <button onClick={() => { setRole(null); setPatient(null); }} className="mt-5 w-full px-3 py-2 rounded-xl text-sm font-semibold text-[var(--ink-soft)] hover:bg-gray-50 flex items-center gap-2">
